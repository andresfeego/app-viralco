...
14. Full-screen React Native modals must explicitly clear the status bar, notch, and Dynamic Island. Do not rely only on the default edges of `SafeAreaView`: obtain `useSafeAreaInsets()`, exclude the top edge from the wrapper, and apply at least `insets.top + tokens.spacing.xl` as the header or content's top padding. This is the mandatory modal clearance and must not be reduced to `tokens.spacing.md`. Preserve the bottom safe area as well. Before delivery, statically verify the inset wiring and ask the user to check the modal on a notched/Dynamic Island simulator or device unless simulator control was explicitly authorized.
