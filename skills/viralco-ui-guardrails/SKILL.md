...
14. Full-screen React Native modals must explicitly clear the status bar, notch, and Dynamic Island. Do not rely only on the default edges of `SafeAreaView`: obtain `useSafeAreaInsets()`, exclude the top edge from the wrapper, and apply `insets.top + tokens.spacing.*` as the header's top padding. Preserve the bottom safe area as well. Before delivery, statically verify the inset wiring and ask the user to check the modal on a notched/Dynamic Island simulator or device unless simulator control was explicitly authorized.
